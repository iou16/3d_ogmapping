...
template <class Cell>
class HierarchicalArray3D: public Array3D< Array3D<Cell> >{
	public:
		...
		void resize(int ixmin, int iymin, int izmin, int ixmax, int iymax, int izmax);
...
HierarchicalArray3D<Cell>::HierarchicalArray3D(int xsize, int ysize, int zsize, int patchMagnitude) 
  :Array3D< Array3D<Cell> >::Array3D((xsize>>patchMagnitude), (ysize>>patchMagnitude), (zsize>>patchMagnitude)){
...
HierarchicalArray3D<Cell>::HierarchicalArray3D(const HierarchicalArray3D& hg)
  :Array3D< Array3D<Cell> >::Array3D((hg.m_xsize>>hg.m_patchMagnitude), (hg.m_ysize>>hg.m_patchMagnitude), (hg.m_zsize>>hg.m_patchMagnitude))
{
	this->m_xsize=hg.m_xsize;
	this->m_ysize=hg.m_ysize;
	this->m_zsize=hg.m_zsize;
	this->m_cells=new Array3D<Cell>**[this->m_xsize];
	for (int x=0; x<this->m_xsize; x++){
		this->m_cells[x]=new Array3D<Cell>*[this->m_ysize];
		for (int y=0; y<this->m_ysize; y++){
      this->m_cells[x][y]=new Array3D<Cell>[this->m_zsize];
...
void HierarchicalArray3D<Cell>::resize(int xmin, int ymin, int zmin, int xmax, int ymax, int zmax){
	...
	int ysize=ymax-ymin;
	int zsize=zmax-zmin;
	Array3D<Cell>*** newcells=new Array3D<Cell>**[xsize];
	for (int x=0; x<xsize; x++){
		newcells[x]=new Array3D<Cell>*[ysize];
		for (int y=0; y<ysize; y++){
			newcells[x][y]=new Array3D<Cell>[zsize];
			for (int z=0; z<zsize; z++)
				newcells[x][y][z]=Array3D<Cell>(0);
		}
	}
...
void HierarchicalArray3D<Cell>::allocActiveArea(){
	for (PointSet::const_iterator it= m_activeArea.begin(); it!=m_activeArea.end(); it++){
		const Array3D<Cell>& ptr=this->m_cells[it->x][it->y][it->z];
		...
		this->m_cells[it->x][it->y][it->z]=Array3D<Cell>(patch);
	}
}
...
bool HierarchicalArray3D<Cell>::isAllocated(int x, int y, int z) const{
  ...
  Array3D<Cell>& ptr=this->m_cells[c.x][c.y][c.z];
	return true;
  // return (ptr != 0);
}
...
Cell& HierarchicalArray3D<Cell>::cell(int x, int y, int z){
	...
	if (!this->m_cells[c.x][c.y][c.z]){
		...
		this->m_cells[c.x][c.y][c.z]=Array3D<Cell>(patch);
	}
	Array3D<Cell>& ptr=this->m_cells[c.x][c.y][c.z];
...
const Cell& HierarchicalArray3D<Cell>::cell(int x, int y, int z) const{
	...
	IntPoint c=patchIndexes(x,y,z);
	const Array3D<Cell>& ptr=this->m_cells[c.x][c.y][c.z];
	return (*ptr).cell(IntPoint(x-(c.x<<m_patchMagnitude),y-(c.y<<m_patchMagnitude),z-(c.z<<m_patchMagnitude)));
}
